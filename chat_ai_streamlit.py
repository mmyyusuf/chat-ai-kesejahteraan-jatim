...
# --- Tambahan Tampilan UI (tidak mengubah logika chatbot) ---
st.markdown("""
    <style>
    /* Background lembut */
    .stApp {
...
# --- Header Identitas ---
st.markdown("""
<div style="background-color:#004d40;padding:15px;border-radius:10px;text-align:center;margin-bottom:20px;">
    <h2 style="color:white;">💬 Chat AI Analisis Kesejahteraan Daerah Jawa Timur</h2>
    <p style="color:#b2dfdb;font-size:16px;">
        Dikembangkan oleh <b>Muhammad Maulana Yusuf</b><br>
        Program Studi Sains Data — UPN Veteran Jawa Timur (Angkatan 2023)
    </p>
</div>
""", unsafe_allow_html=True)
...
with col1:
    ...
    if user_input:
        ...
        # ---- FITUR BARU: Tampilkan hasil cluster sebagai gambar ----
        if any(x in ui for x in ["tampilkan hasil cluster", "berikan hasil cluster", "visualisasi cluster", "gambar cluster"]):
            try:
                st.subheader("🗺️ Visualisasi Hasil Cluster Kabupaten/Kota")
                st.image("visualisasi_cluster.png", caption="Visualisasi Cluster Agglomerative 2023", use_container_width=True)
            except Exception as e:
                st.error("Gagal menampilkan gambar. Pastikan file 'visualisasi_cluster.png' ada di folder yang sama.")
        
        # ---- Jelaskan cluster ----
        elif any(x in ui for x in ["jelaskan", "penjelasan", "deskripsi"]) and "cluster" in ui:
